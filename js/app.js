...
new _vue2.default({
  ...
  ready: function ready() {
    ...
    this.$http.get('data.json').then(function (res) {
      ...
      _this.createWaypoint("islam-fact-check", true);
      setTimeout(function () {
        var _loop = function _loop(i) {
          _this.createWaypoint(_this.infos[i].categoryId, false, _this.infos[i].category);

          var _loop2 = function _loop2(j) {
            var id = _this.infos[i].data[j].id;
            _this.createWaypoint(id, false);
            var factEl = document.getElementById(_this.infos[i].data[j].factid);
            var factElPrint = document.getElementById(_this.infos[i].data[j].factid + 'print');
            var factElHidden = document.getElementById(_this.infos[i].data[j].factid + 'hidden');
            factEl.innerHTML = _this.infos[i].data[j].fact.replace('FACT:', '');
            factEl.innerHTML = factEl.innerHTML.substring(0, _this.infos[i].data[j].cutoffPoint);
            factElHidden.innerHTML = _this.infos[i].data[j].fact.replace('FACT:', '');
            factElPrint.innerHTML = _this.infos[i].data[j].fact;
            var inPageLink = encodeURIComponent('http://islamfactcheck.org/#' + id);
            _this.$http.get('https://api-ssl.bitly.com/v3/shorten?access_token=b30f7be0c9aee687f8410b27d14a9afd4a6d5cdf&longUrl=' + inPageLink).then(function (res) {
              _this.infos[i].data[j].inPageLink = res.data.data.url;
            });
          };

          for (var j in _this.infos[i].data) {
            _loop2(j);
          }
        };

        for (var i in _this.infos) {
          _loop(i);
        }
  ...
  methods: {
    ...
    gotoCategory: function gotoCategory(categoryId) {
      window.location.hash = categoryId;
      this.hideOverlay();
    },
    toggleMoreText: function toggleMoreText(showMore) {
      setTimeout(function () {
        return Waypoint.refreshAll();
      }, 10);
      return !showMore;
    },
    createWaypoint: function createWaypoint(id, removeHash, category) {
      var self = this;
      new Waypoint({
    ...
    share: function share(shareTo, data) {
      var tempEl = document.createElement('div');
      tempEl.innerHTML = data.fact;
      data.fact = tempEl.innerText;
      var text = 'Assertion: ' + data.assertion + '\n\r' + data.fact;
      if (shareTo === 'Facebook') {
        ...
        var body = encodeURIComponent('I saw this on Islam Fact Check and though you might like it.\n\r' + text).substr(0, 1500);
        body += encodeURIComponent('...\n\rRead more at ' + data.inPageLink + '\n\rThanks!');
        this.shareViaMail(subject, body);
      }
    },
    shareViaFacebook: function shareViaFacebook(link) {
    ...
    print: function print(data) {
      ...
      if (data === undefined) {
        (function () {
          _this2.printAll = true;
          var currentPageTitle = _this2.pageTitleEl.innerText;
          setTimeout(function () {
            window.print();
            _this2.printAll = false;
            _this2.pageTitleEl.innerText = currentPageTitle;
          }, 100);
        })();
      } else {
...
},{"material-design-lite/material":2,"vue":28,"vue-resource":17,"waypoints/lib/noframework.waypoints":29}],2:[function(require,module,exports){
;(function() {
"use strict";
